...
const Purchase = () => {
  ...
  useEffect(() => {
    fetch(`https://toystore-server-side.vercel.app/toys/${toyId}`)
      .then((res) => res.json())
      .then((data) => {
        setToy(data);
        setOrder(data);
  ...
  const handleOrder = (order) => {
    ...
    fetch(`https://toystore-server-side.vercel.app/addOrders`, {
      method: "POST",
    ...
    <Container className="my-4">
      ...
      <div className="col-md-12">
        <Form
          ...
          className="d-flex flex-column justify-content-center align-items-center h-100 col-md-6 mx-auto"
        >
          ...
          />
          <Button variant="warning" onClick={() => handleOrder(order)}>
            Order Now
          </Button>
        </Form>
      </div>
